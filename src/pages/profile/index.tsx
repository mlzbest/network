...
export default function ProfilePage() {
  ...
  const resetInactivityTimer = () => {
    ...
    [DEV](`[profile] 🔥 [${timeStr}] 启动120秒计时器(setInterval模式), timerStartTimeRef=${now}`);
    ...
    inactivityTimerRef.current = setInterval(() => {
      ...
      if (currentPage !== 'profile') {
        [DEV](`[profile] ⚠️ 当前活跃页面是${currentPage},不是profile,清除计时器`);
        if (inactivityTimerRef.current) {
          clearInterval(inactivityTimerRef.current);
          inactivityTimerRef.current = null;
        }
      ...
      if (elapsed >= 120000) {
        ...
        [DEV](`[profile] ⏰ [${timeoutTime}] 确认超时(${Math.floor(elapsed / 1000)}秒),执行跳转`);
        // 先清除计时器并重置时间戳,防止重复跳转
        if (inactivityTimerRef.current) {
          clearInterval(inactivityTimerRef.current);
          inactivityTimerRef.current = null;
  ...
  const checkTimeout = () => {
    ...
    const timeStr = new Date().toLocaleTimeString();
    [DEV](`[profile] 🔍 [${timeStr}] 检查超时: 已过去 ${Math.floor(elapsed / 1000)}秒`);
    if (elapsed >= 120000) {
      [DEV](`[profile] ⚠️ [${timeStr}] 检测到已超时(${Math.floor(elapsed / 1000)}秒),立即跳转`);
      Taro.reLaunch({ url: '/pages/ping/index' });
    } else {
      const remaining = Math.ceil((120000 - elapsed) / 1000);
      [DEV](`[profile] ℹ️ [${timeStr}] 未超时,剩余 ${remaining}秒`);
    }
  };
  ...
  useDidShow(() => {
    ...
    if (isShieldArmed() && getArmedRoute() !== 'pages/profile/index') {
      [DEV]('[profile] 检测到遮挡未解除, 兜底弹回:', getArmedRoute());
    ...
    if (inactivityTimerRef.current) {
      ...
      inactivityTimerRef.current = null;
      [DEV](`[profile] 🛑 [${timeStr}] 强制清除旧计时器`);
    }

    setCurrentPage('profile'); // 记录当前活跃页面

    [DEV](`[profile] 📱 [${timeStr}] useDidShow 触发, currentPage=${currentPage}, isPageSwitch=${isPageSwitch}`);
    setTimerStatus('已启动');

    if (isPageSwitch) {
      // 页面切换:重置计时器
      [DEV](`[profile] 🔄 [${timeStr}] 检测到页面切换(从${currentPage}切换过来),重置计时器`);
      resetInactivityTimer();
    } else if (timerStartTimeRef.current) {
      // 从后台切回:检查是否超时
      [DEV](`[profile] 🔙 [${timeStr}] 从后台切回,检查是否超时`);
      checkTimeout();
    } else {
      // 首次进入页面,启动计时器
      [DEV](`[profile] 🆕 [${timeStr}] 首次进入页面,启动计时器`);
      resetInactivityTimer();
    }
  ...
  useDidHide(() => {
    const timeStr = new Date().toLocaleTimeString();
    [DEV](`[profile] 👻 [${timeStr}] useDidHide 触发,清除计时器并标记为页面切换`);
    setTimerStatus('页面隐藏');
    ...
    if (inactivityTimerRef.current) {
      clearTimeout(inactivityTimerRef.current);
      inactivityTimerRef.current = null;
      [DEV](`[profile] 🛑 [${timeStr}] 已清除旧计时器`);
    }

    setPageSwitching(true); // 标记为页面切换
    [DEV](`[profile] 🏷️ [${timeStr}] 已调用setPageSwitching(true)`);
  });
